...
async def _upload_product_image(img: UploadFile) -> str:
    """Shared by add-product and edit-product below. Re-encodes through
    Pillow server-side (WebP) instead of trusting the client's filename
    extension / Content-Type — this both normalizes storage and doubles as
    content validation, since a non-image payload fails to decode and is
    rejected with a 400 rather than uploaded as-is."""
...
async def add_product(
    ...
    try:
        ...
        for img in valid_images[:MAX_TOTAL_IMAGES]:
            image_urls.append(await _upload_product_image(img))
...
async def edit_product(
    ...
    try:
        ...
        for img in valid_new[:slots_remaining]:
            new_urls.append(await _upload_product_image(img))
